...
import VoyageInformation from '../../src/ocean-fishing/VoyageInformation'
import AchievementsInformation from '../../src/ocean-fishing/AchievementsInformation'
import { Destination, Time } from '../../src/ocean-fishing/ffxiv-ocean-fishing'
...
const OceanFishing = (): React.ReactElement => {
    ...
    const [selectedVoyage, setSelectedVoyage] = useState<{ destination: Destination, time: Time } | null>(null) // This is initialized when UpcomingVoyages is mounted
    ...
    const onSelectVoyage = (destination: Destination, time: Time) => {
        setSelectedVoyage({ destination, time })
    }
    ...
    return (
        <Page title={t('_title')} description={t('_description')}>
            ...
            <ChecklistContext.Provider value={{ checklist: checklist ?? [], setChecklist }}>
                ...
                <UpcomingVoyages now={now} checklist={checklist} onSelectVoyage={onSelectVoyage} />
                {selectedVoyage != null && (
                    <>
                        <VoyageInformation now={now} destination={selectedVoyage.destination} time={selectedVoyage.time} />
                        <AchievementsInformation destination={selectedVoyage.destination} time={selectedVoyage.time} />
                    </>
                )}
