...
const TimeIcon = ({ time, sx }: Props): React.ReactElement => {
  const { t } = useTranslation('ocean-fishing')

  switch (time) {
    case 'D':
      return (
        <Tooltip title={String(t('time.day'))}>
          <Icon width={ICON_SIZE} height={ICON_SIZE} sx={sx}>
    ...
    case 'S':
      return (
        <Tooltip title={String(t('time.sunset'))}>
          <Icon width={ICON_SIZE} height={ICON_SIZE} sx={sx}>
    ...
    case 'N':
      return (
        <Tooltip title={String(t('time.night'))}>
          <Icon width={ICON_SIZE} height={ICON_SIZE} sx={sx}>
